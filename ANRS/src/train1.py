'''
train process without tensorboard
'''
import logging
import re
...
import time
import numpy as np
from config import model_name, get_params
...
def train(params):
    ...
    with tqdm(total=Config.num_batches, desc="Training") as pbar:
        for i in range(1, Config.num_batches + 1):
            ...
            y_pred, aspect_classification_loss = model(
            ...
            if model_name == 'ANRS' :
                loss += Config.aspect_classification_loss_weight * aspect_classification_loss
            ...
            if i % Config.num_batches_validate == 0:
                val_auc, val_mrr, val_mrr5, val_mrr10, val_ndcg, val_ndcg5, val_ndcg10, \
                val_map, val_map5, val_map10, val_recall, val_recall5, val_recall10, \
                val_precision, val_precision5, val_precision10 = evaluate(
                    model, 'data/val_l')

                tqdm.write(
                    f"Time {time_since(start_time)}, batches {i}, validation AUC: {val_auc:.4f}, validation MRR: {val_mrr:.4f},"
                    f"validation MRR5: {val_mrr5:.4f}, validation MRR10: {val_mrr10:.4f},"
                    f"validation nDCG: {val_ndcg:.4f}, validation nDCG@5: {val_ndcg5:.4f}, validation nDCG@10: {val_ndcg10:.4f}, "
                    f"validation MAP: {val_map:.4f}, validation MAP@5: {val_map5:.4f}, validation MAP@10: {val_map10:.4f}, "
                    f"validation Recall: {val_recall:.4f}, validation Recall@5: {val_recall5:.4f}, validation Recall@10: {val_recall10:.4f}, "
                    f"validation Precision: {val_precision:.4f}, validation Precision@5: {val_precision5:.4f}, validation Precision@10: {val_precision10:.4f}"
                )
