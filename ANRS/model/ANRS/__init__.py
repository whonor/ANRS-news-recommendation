...
from model.ANRS.AEmodel import abae, max_margin_loss, orthogonal_regularization
from model.ANRS.news_encoder import NewsEncoder
from model.ANRS.user_encoder import UserEncoder
from model.ANRS.dot_product import DotProductClickPredictor
from model.ANRS.w2v import word2vec
...
class ANRS(torch.nn.Module):
    ...
    def forward(self, candidate_news, clicked_news):
        ...
        # 1 + K, batch_size, num_filters [5, 128, 300]
        candidate_news_vector = torch.stack(
            [self.news_encoder(x) for x in candidate_news])
        # batch_size, num_clicked_news_a_user, num_filters [128, 50, 300]
        clicked_news_vector = torch.stack(
            [self.news_encoder(x) for x in clicked_news], dim=1)
        
        # shape: rs[1000, 300]; zs[1000, 300]; zn[128, 300]
        ...
        # crs[100, 300], czs[100, 300], czn[128, 300]
        ...
        U = orthogonal_regularization(self.aspect_encoder.T.weight)
        loss = J + self.config.ortho_reg * self.config.batch_size * U
